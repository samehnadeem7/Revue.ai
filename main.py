...
async def upload_pdf(
    ...
    try:
        ...
        # Update metrics
        cursor.execute('''
            INSERT OR REPLACE INTO user_metrics (document_type, analysis_count, last_analyzed)
            VALUES (?, 
                    COALESCE((SELECT analysis_count + 1 FROM user_metrics WHERE document_type = ?), 1),
                    CURRENT_TIMESTAMP)
...
async def get_history(limit: int = 10):
    ...
    cursor.execute('''
        SELECT filename, document_type, created_at, analysis_id
